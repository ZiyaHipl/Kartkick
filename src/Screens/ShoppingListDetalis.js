...
import {  Text,View,FlatList,StyleSheet,Image,TouchableOpacity,ScrollView,DeviceEventEmitter} from "react-native";
...
export default class ShoppingListDetalis extends Component {
    constructor(props) {
        super(props);
        this.state = {
            ProductList: '',
    ...
    shoppinglist_item_create = () => {
        ...
        var data = {
            items: this.props.route.params.Product,
            quantity:this.state.count,
        }
         console.log('>>>====++++',JSON.stringify(data))
        Helper.showLoader()
        Helper.makeRequest({ url: ApiUrl.shoppinglist_item_create, method: "POST", data: data }).then((respoanse) => {
            ...
            <ScrollView style={{ flex: 1, backgroundColor: '#fff' }} >
                ...
                <View>
                    ...
                    <View style={{ marginTop: 20, flexDirection: "row", marginHorizontal: 16, alignItems: "center", }}  >
                        ...
                        <Text style={styles.txtMoney}>$ {this.state.ProductList?Number(this.state.ProductList.price):''}</Text>
    ...
    GetProductDetails = () => {
        ...
        Helper.makeRequest({ url: ApiUrl.shoppinglist_item + this.state.Productitem+'/', method: "GET", data: '' }).then((respoanse) => {
           console.log('respoanse====>',respoanse.data.product)
            Helper.hideLoader()
            if (respoanse.status){
                this.setState({ ProductList: respoanse.data.product.items, like: respoanse.data.like,NotFound:false,count:respoanse.data.product.quantity?respoanse.data.product.quantity:1 })
            }else{
