...
import { Animated, StyleSheet, View, Text, ImageBackground, TouchableOpacity, Image,TextInput } from 'react-native';
...
export default class RemoveCart extends Component {
    ...
    renderDown = ({ item, index }) => {
        return (
            <View style={this.state.isOpen ? styles.mainView2 : styles.mainView21}>
                <View>
                    <Image style={styles.icon2} source={{ uri: Config.baseImgurl + item.shoppinglist1.items.image }} />
                </View>
                <View style={{ flex: 1, marginLeft: 6 }}>
                    <Text style={styles.textType1}>{item.shoppinglist1.items.sub_category} </Text>
                ...
                <Text style={styles.textStore}>{item.store}</Text>
                    ...
                    <View style={{ flexDirection: "row", alignItems: "center", marginTop: 5 }}>
                        <TouchableOpacity
                            onPress={() => { this.decrement(item, index) }}  >
                            <Image style={styles.touch2} source={images.minus} />
                        </TouchableOpacity>

                        <Text style={styles.number2}>{item.shoppinglist1.quantity}</Text>
                ...
                <View>
                    ...
                    <ImageBackground style={styles.ImageBack} source={images.ribbon}>
                        <Text style={styles.textPrice1}>${(Number(item.shoppinglist1.quantity) * Number(item.shoppinglist1.items.price)).toFixed(2)} </Text>
                    </ImageBackground>

                    {/* <TouchableOpacity hitSlop={{ top: 30, bottom: 30, left: 30, right: 30 }} onPress={() => { this.is_private(item, index) }}>
                <Image style={[styles.lockIcon, { tintColor: item.shoppinglist1.is_private ? Colors.purple : Colors.pinkishGrey }]} source={images.lock} />
              </TouchableOpacity> */}
    ...
    //             <TouchableOpacity
    //                 onPress={() => {  this.props.showModalSelectedSubstitues();  }} style={styles.slideBttnviewLeft} >
    ...
    renderRightActions = () => {
        return (
            <RectButton style={styles.rightAction} onPress={this.close}>
                <TouchableOpacity onPress={() => { this.props.Removeitem('ok') }} style={styles.slideBttnview}>
            ...
            <View style={{ flex: 1, marginHorizontal: 15 }}>
                <Swipeable
                    ...
                    renderRightActions={({ item, index }) => <RectButton style={styles.rightAction} onPress={this.close}>
                        <TouchableOpacity onPress={() => this.props.Removeitem(this.props.item)} style={styles.slideBttnview}>
                            <Image style={styles.removeIcon} source={images.remove} />
                            <Text style={styles.removeText}>Remove {"\n"}   Item</Text>
                        </TouchableOpacity>
                    </RectButton>}
                    ...
                    onSwipeableOpen={() => { }}
                >
                    <TouchableOpacity onPress={() => { }} style={[styles.mainView21, { marginLeft: !item.isOpeen ? 0 : 15, marginRight: !item.isOpeen ? 0 : 15, },]} >

                        <View>
                            <Image style={[styles.icon2, { marginRight: 10 }]} source={{ uri: Config.baseImgurl + item.shoppinglist1.items.image }} />
                        </View>
                        <View style={{ flex: 1, marginLeft: 5 }}>
                            <Text style={[styles.textType1, { marginTop: 5 }]}>{item.shoppinglist1.items.sub_category} </Text>

                            <View style={{ flexDirection: "row", alignItems: "center", marginTop: 5 }}>
                                <TouchableOpacity onPress={() => { this.props.decrement1(item, index); }}  >
                                    <Image style={styles.touch2} source={images.minus} />
                                </TouchableOpacity>
                                <Text style={styles.number2}>{item.shoppinglist1.quantity}</Text>
                                {/* <TextInput
                                    ...
                                    onChangeText={(text) => { this.props.directChange(item, index, text) }}
                                    maxLength={3}
                                    keyboardType={'number-pad'}
                                    value={item.shoppinglist1.quantity == "" ? '0': item.shoppinglist1.quantity.toString()} /> */}
                                <TouchableOpacity onPress={() => { this.props.increment1(item, index); }} >
                                    <Image style={styles.touch2} source={images.plus} />
                                </TouchableOpacity>
                            </View>
                        </View>
                        <View>
                            <ImageBackground style={styles.ImageBack} source={images.ribbon}>
                                <Text style={{ marginTop: 3, alignSelf: "center", fontSize: 16, lineHeight: 20, color: Colors.white, }}  >
                                    ${(Number(item.shoppinglist1.quantity) * Number(item.shoppinglist1.items.price)).toFixed(2)}
                                    {/* { item.number == 0 ? item.price :  (Number(item.sub_category.quantity) * Number(item.sub_category.items.price)).toFixed(2)  } */}
                                </Text>
                            </ImageBackground>
                            {/* <TouchableOpacity hitSlop={{top: 30, bottom: 30, left: 30, right: 30 }} onPress={() => { this.props.is_private(item,index)}} >
                          <Image style={[styles.lockIcon,{tintColor:item.sub_category.is_private? Colors.purple :Colors.pinkishGrey }]} source={images.lock} />
                        </TouchableOpacity> */}
                        </View>
                    </TouchableOpacity>
                </Swipeable>
...
const styles = StyleSheet.create({
    ...
    number2: { paddingHorizontal: 10, backgroundColor: Colors.white ,textAlign:'center'},
