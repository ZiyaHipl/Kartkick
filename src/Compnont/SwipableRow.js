...
import { Animated, StyleSheet, View, Text, ImageBackground, TouchableOpacity, Image, TextInput, DeviceEventEmitter } from 'react-native';
...
export default class SwipableRow extends Component {
    ...
    render() {
        const { item, index, tempData } = this.props
        return (
            <View style={{ flex: 1, marginHorizontal: 15 }}>
                <Swipeable
                    ...
                    onSwipeableOpen={() => { }}
                >
                    <TouchableOpacity onPress={() => { this.props.ProductDetails(item, index); }} style={[styles.mainView21, { marginLeft: !item.isOpeen ? 0 : 15, marginRight: !item.isOpeen ? 0 : 15, borderWidth: 2, borderColor: tempData.includes(item.sub_category.id) ? Colors.barney : 'transparent' },]} >

                        <View>
                            <Image style={[styles.icon2, { marginRight: 10 }]} source={{ uri: Config.baseImgurl + item.sub_category.items.image }} />
                        </View>
                        <View style={{ flex: 1, marginLeft: 5 }}>
                            <Text style={[styles.textType1, { marginTop: 5 }]}>{item.sub_category.items.sub_category} </Text>
                            ...
                            <View style={{ flexDirection: "row", alignItems: "center", marginTop: 5 }}>
                                ...
                                {/* {item.sub_category.quantity.length <= 0 ?
                                ...
                                {/* <TextInput
                                    ...
                                    value={item.sub_category.quantity == "" ? '0' : item.sub_category.quantity.toString()} /> */}
                                <Text style={styles.number2}>{item.sub_category.quantity}</Text>
                        ...
                        <View>
                            <ImageBackground style={styles.ImageBack} source={images.ribbon}>
                                <Text style={{ marginTop: 3, alignSelf: "center", fontSize: 16, lineHeight: 20, color: Colors.white, }}  >
                                    {/* {item.number == 0 ? item.price : Number(item.sub_category.items.price).toFixed(2)} */}
                                    {item.sub_category.items.price == 0 ? item.price : (Number(item.sub_category.quantity) * Number(item.sub_category.items.price)).toFixed(2)}
                                </Text>
                            </ImageBackground>
